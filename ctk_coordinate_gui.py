...
from ctk_messagebox import CTkMessagebox
...
class GlobeMap:
    ...
    def load_state(self):
        try:
            with open("canvas_state.json", "r") as f:
                ...
                # self.hbar.set(*state["hbar"])
                # self.vbar.set(*state["vbar"])
    ...
    def __bind_canvas_events(self):
        ...
        self.canvas.bind("<B1-Motion>", self.update_mouse_coord, add="+")
    ...
    def __wheel(self, event):
        ...
        # if (
        #     hasattr(self, "get_coordinate")
        #     and hasattr(self.get_coordinate, "rect")
        #     and hasattr(self.get_coordinate.rect, "lon_min")
        # ):

        # else:
        # print("kok ga ketemu ya lonmin")
...
class GridLines:
    def __init__(self, mainmap: GlobeMap, pingx1, pingy1):
        ...
        self.draw_grid_lines(pingx1=pingx1, pingy1=pingy1)

    def degree_to_pixel(self, lon=0, lat=0):
        return self.globemap.degree_to_pixel(lon, lat)

    def draw_grid_lines(self, pingx1, pingy1):
        ...
        # Draw longitude annotations
        for lon in lon_range:
            x, y = self.degree_to_pixel(lon=lon)
            if grid_limit[0] <= x <= (grid_limit[2] + 2):
                self.create_longitude_text(x, pingy1, lon)

        # Draw latitude annotations
        for lat in lat_range:
            x, y = self.degree_to_pixel(lat=lat)
            if grid_limit[1] <= y <= (grid_limit[3] + 2):
...
class CoordinateWidget:
    def __init__(self, mainmap: GlobeMap, given_coords: list[float]):
        self.globemap = mainmap
        self.canvas = mainmap.canvas
        self.status = ["coordinate not inputed yet"]
    ...
    def validate(self, *args):
        ...
        if not self.is_valid_number((self.roi[0].get()), -180, 180):
            ...
            self.status += ["invalid coordinate value"]
    ...
    def replot_button(self):
        state = (
            tk.NORMAL if "invalid coordinate value" not in self.status else tk.DISABLED
        )
...
class ResizeableRectangle:
    def __init__(self, mainmap: GlobeMap, coord_widget: CoordinateWidget):
        ...
        self.roi[0].trace_add("write", self.coord_widget.validate)
        self.roi[1].trace_add("write", self.coord_widget.validate)
        self.roi[2].trace_add("write", self.coord_widget.validate)
        self.roi[3].trace_add("write", self.coord_widget.validate)
    ...
    def selecting_line_to_resize(self, event):
        # self.coord_widget.update_mouse_coord(event)
        x = self.canvas.canvasx(event.x)
        y = self.canvas.canvasy(event.y)
    ...
    def rect_resize_click(self, event):
        ...
        self.canvas.bind("<B1-Motion>", self.rect_resize_drag, add="+")
        self.canvas.bind("<B1-Motion>", self.coord_widget.update_mouse_coord, add="+")
    ...
    def rect_create_click(self, event):
        ...
        print("start selec rect dipanggil")
        self.canvas.bind("<ButtonRelease-1>", self.rect_release)
        self.canvas.bind("<B1-Motion>", self.coord_widget.update_mouse_coord, add="+")
        self.canvas.bind("<B1-Motion>", self.rect_create_drag, add="+")
    ...
    def rect_create_drag(self, event):
        ...
        self.refresh_roi(False)

    def draw_rect_manually(self, job=None):

        if "invalid coordinate value" in self.coord_widget.status:
            pass
        elif "coordinate not inputed yet" in self.coord_widget.status:
            pass
        else:
            global rectangle
    ...
    def refresh_roi(self, sort=True, **kwargs):
    ...
    def update_roi_from_entry(self):
        ...
        self.coord_widget.x_min_entry.bind(
            "<Return>", lambda event: self.draw_rect_manually("edit")
        )
        self.coord_widget.x_max_entry.bind(
            "<Return>", lambda event: self.draw_rect_manually("edit")
        )
...
class CoordWindow(ctk.CTkToplevel):
    ...
    def check_before_close(self):
        ...
        if "coordinate not inputed yet" in self.globemap.get_coordinate.status:
            mesage = f"Coordinate not inputed yet,\nDo you want to exit?"
            icon = "warning"
            self.closing(mesage, icon)
        elif "invalid coordinate value" not in self.globemap.get_coordinate.status:
            x1, x2 = anot(
        ...
        elif "invalid coordinate value" in self.globemap.get_coordinate.status:
            mesage = f"Coordinate not valid,\nDo you want to exit?"
            icon = "cancel"
            self.closing(mesage, icon)
