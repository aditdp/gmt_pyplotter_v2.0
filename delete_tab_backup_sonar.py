...
class GrdOptions:
    ...
    @property
    def unit(self):
        return self.__unit[self.grd.get()][0]

    @property
    def type(self):
        return self.__unit[self.grd.get()][1]
...
class GrdImage(ColorOptions):
    ...
    def script(self):
        ...
        return f"{makecpt}\tgmt grdimage {remote_data} {shade} -C{cpt_file} {sea_mask}"
...
"""
    ...
    def depth_legend(self):
        ...
        if min_depth != 0:
            fg = ""
        depth_colorbar_plot = f"\tgmt colorbar -DJBC{offset}{width}+h+e{fg}0.3c -C{self.name}-depth.cpt {depth_label} -Bx+lEq_depth -By+lkm --FONT_LABEL=15p --FONT_ANNOT=15p --MAP_FRAME_PEN=0.75p\n"
